    ...
    override func viewDidLoad() {
        ...
        viewModel.showDetalView
            ...
            .drive(onNext: { [weak self] gitHubRepository in
                ...
                // detailHeading, detailCountでStateは共有
                let detailHeadingViewModel = RepositoryDetailHeadingViewModelImpl(store: AppContainer.shared.store, state: AppContainer.shared.store.stateObservable.flatMap { $0.repositoryDetailState == nil ? Observable.empty() : Observable.just($0.repositoryDetailState!) })
                let detailHeadingViewController = RepositoryDetailHeadingViewController.build(viewModel: detailHeadingViewModel)

                let detailCountViewModel = RepositoryDetailCountViewModelImpl(store: AppContainer.shared.store, state: AppContainer.shared.store.stateObservable.flatMap { $0.repositoryDetailState == nil ? Observable.empty() : Observable.just($0.repositoryDetailState!) })
