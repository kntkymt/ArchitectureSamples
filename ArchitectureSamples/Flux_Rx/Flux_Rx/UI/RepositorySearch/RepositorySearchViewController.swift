    ...
    override func viewDidLoad() {
        ...
        viewModel.showDetalView
            ...
            .drive(onNext: { [weak self] gitHubRepository in
                ...
                // detailHeading, detailCountでStore, Dispatcher, ActionCreatorは共通にする
                // 画面とFluxのコンポーネントが1:1でなくても良い例
                let dispatcher = RepositoryDetailDispatcher()
                let store = RepositoryDetailStore(gitHubRepository: gitHubRepository, dispatcher: dispatcher)
                let actionCreator = RepositoryDetailActionCreator(dispatcher: dispatcher)

                let detailHeadingViewModel = RepositoryDetailHeadingViewModelImpl(repositoryDetailStore: store, repositoryDetailActionCreator: actionCreator)
                let detailHeadingViewController = RepositoryDetailHeadingViewController.build(viewModel: detailHeadingViewModel)

                let detailCountViewModel = RepositoryDetailCountViewModelImpl(repositoryDetailStore: store, repositoryDetailActionCreator: actionCreator)
                let detailCountViewController = RepositoryDetailCountViewController.build(viewModel: detailCountViewModel)
